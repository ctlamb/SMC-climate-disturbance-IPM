...
ggplot() +
  geom_ribbon(
    ...
    aes(x = yrs, y = totN, ymin = totN.lower, ymax = totN.upper), alpha = 0.4, color = NA, fill = cols[3]
  ) +
  geom_line(
    data = demog %>%
      ungroup() %>%
    ...
    aes(x = yrs, y = totN, ymin = totN.lower, ymax = totN.upper), size = 1, color = cols[3]
  ) +
  geom_rug(
    data = raw.demog %>%
      ungroup() %>%
...
lambda.mod <- demog.draws %>%
  group_by(.draw) %>%
  do(tidy(glm(log(lambda) ~ dist.hum_scale + snow.depth_scale + start.spring_scale + warm.dry.summer_scale + rain.on.snow_scale, data =.))) %>%
  filter(!term %in% "(Intercept)")%>%
  group_by(term)%>%
  summarise(.value=median(estimate),
            .lower=(quantile(estimate,0.025)),
            .upper=(quantile(estimate,0.975)))%>%
  mutate(var=case_when(term=="dist.hum_scale"~"dist",
...
lambda.mod.mm <- demog.draws %>%
  group_by(.draw) %>%
  do(tidy(lmer(log(lambda) ~ dist.hum_scale + snow.depth_scale + start.spring_scale + warm.dry.summer_scale + rain.on.snow_scale + (1|herd), data =.))) %>%
...
#     dist = mean(dist.hum_scale, na.rm = TRUE),
...
mod.dat.ch.50 <- mod.dat.ch%>%pivot_wider(names_from="CH_type", values_from=dist)%>%filter(N>50)

ggplot(mod.dat.ch.50,
...
mod.dat.ch.50%>%
  filter(!herd%in%c("Maligne", "Tonquin"))%>% ##no CH delineated
  ungroup%>%
  mutate(HEWSR.cat=cut_width(HEWSR, 0.05),
         Matrix.cat=cut_width(`LEWSR/Matrix`, 0.1)
         )%>%
  group_by(HEWSR.cat,Matrix.cat)%>%
  summarise(lambda=exp(log(r)%>%mean),
            n=n(),
            ss=sum(r>=1)/n)%>%
  ggplot()+
