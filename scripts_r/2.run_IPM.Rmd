...
meansr[1,2] <- 10000 ##make sex ratio more precise so model doesnt eat variance up here
...
draws%>%
  ...
  geom_vline(xintercept = 0, linetype="dashed")
```
```{r plot Nakusp for Nowak, results='hide', message=FALSE, warning=FALSE}
df.temp <- rbind(edat%>%mutate(type="count", se=sqrt(1/tau))%>%select(herd,year,mu,se,type),
      sdat%>%mutate(mu=(1-(events/trials))^365,se=sqrt((mu*(1-mu))/(trials/365)),type="S")%>%select(herd,year,mu,se,type),
      rdat%>%mutate(mu=events/trials,se=sqrt((mu*(1-mu))/trials),type="R")%>%select(herd,year,mu,se,type))%>%
  filter(herd=="23")
...
ipm.temp <- demog%>%
  filter(i==23)%>%
  mutate(type="R")%>%
  select(herd,year=yrs,mu=R, lower=R.lower, upper=R.upper, type)%>%
  rbind(demog%>%
  filter(i==23)%>%
  mutate(type="S")%>%
  select(herd,year=yrs,mu=S, lower=S.lower, upper=S.upper, type))%>%
  rbind(demog%>%
  filter(i==23)%>%
  mutate(type="count")%>%
  select(herd,year=yrs,mu=totNMF, lower=totNMF.lower, upper=totNMF.upper, type))%>%
  rbind(demog%>%
  filter(i==23)%>%
  mutate(type="lambda")%>%
  select(herd,year=yrs,mu=lambda, lower=lambda.lower, upper=lambda.upper, type))


  ggplot()+
  geom_linerange(data=df.temp, aes(x=year+1969, y=mu, ymin=mu-se, ymax=mu+se))+
  geom_point(data=df.temp, aes(x=year+1969, y=mu, ymin=mu-se, ymax=mu+se))+
  geom_linerange(data=ipm.temp, aes(x=year, y=mu, ymin=lower, ymax=upper), color="orange")+
  geom_point(data=ipm.temp, aes(x=year, y=mu,  ymin=lower, ymax=upper), color="orange")+
...
covariates%>%
  filter(year>1970, herd=="Nakusp")%>%
  select(herd,year,dist.hum, snow.depth, start.spring, warm.dry.summer, rain.on.snow)%>%
  pivot_longer(dist.hum:rain.on.snow)%>%
...
df.temp%>%
  select(year,mu,type)%>%
  pivot_wider(values_from=mu, names_from=type)%>%
  select(-year)%>%
  correlate()
