...
## Line up years so that it all affects animals by March year X count. So summer and spring need to be X+1
```{r filter recovery actions, results='hide', message=FALSE, warning=FALSE}

covar.names <- covariates%>%select(-herd,-year,-herd_num)%>%colnames ##remove herd and year
...
survey_timing <- recruit.yr.ipm %>%
  ...
  rbind(expand.grid(
    ...
    year=(1:nyr)[!(1:nyr)%in%unique(afr%>%
